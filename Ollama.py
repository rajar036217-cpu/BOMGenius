    ...
    if ebom_file and inv_file:
        ...
        if st.button("Generate MBOM"):
            ...
            if results:
                ...
                # We skip the markdown conversion and just display the calculated results directly as a table
                st.dataframe(df_results, use_container_width=True)
                
                # Optional: Button to download the MBOM as CSV
                csv = df_results.to_csv(index=False).encode('utf-8')
                st.download_button(
                    label="Download MBOM as CSV",
                    data=csv,
                    file_name="manufacturing_bom.csv",
                    mime="text/csv",
                )
